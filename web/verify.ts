...
module.exports = {
  ...
  run: async (client:Discord.Client, req:http.IncomingMessage, res:http.ServerResponse, parsed:url.UrlWithParsedQuery) => {
    if (!parsed.query || !parsed.query.token || !(client as any).verifyQueue.get(parsed.query.token)) {
      ...
      fs.readFile('./assets/html/verify.html', 'utf8', (err:any, data:string) => {
        res.end(data
          ...
          .replace('{message}', require('/home/azureuser/verifier/data/config.json').guilds[(client as any).verifyQueue.get(parsed.query.token).guild.id] ? (require('/home/azureuser/verifier/data/config.json').guilds[(client as any).verifyQueue.get(parsed.query.token).guild.id].msg ? require('/home/azureuser/verifier/data/config.json').guilds[(client as any).verifyQueue.get(parsed.query.token).guild.id].msg : '{서버.이름} 규칙에 동의하신다면 아래 reCAPTCHA를 풀어주세요.')
          ...
          .replace(/</gi, '&lt;')
          .replace(/>/gi, '&gt;')
          .replace(/{줄바꿈}/gi, '<br>') : `${(client as any).verifyQueue.get(parsed.query.token).guild.name} 규칙에 동의하신다면 아래 reCAPTCHA를 풀어주세요.`)
        )
      })
    }
  }
